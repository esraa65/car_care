                      ...
                      child: Column(
                        children: [
                          ...
                          CustomWorkShopDetails(
                            subtitle: widget.entity.reservationDate.toString(),
                            title: "تاريخ الحجز",
                          ),
                          height(16),
                          ...
                          height(16),
                          CustomWorkShopDetails(
                            subtitle: widget.entity.services.isEmpty
                                ? "لايوجد خدمات"
                                : widget.entity.services.first.name,
                            title: "فئة الخدمة",
                          ),
                          height(16),
                          const Divider(),
                          height(16),
                          SizedBox(
                            height: widget.entity.services.isEmpty
                                ? 10
                                : widget.entity.services.length * 40 +
                                    (widget.entity.services.length - 1),
                            width: double.infinity,
                            child: ListView.separated(
                              itemBuilder: (ctx, index) {
                                return CustomRowWorkshopDetailsWithIcon(
                                    title: widget.entity.services[index].name,
                                    price:
                                        "${widget.entity.services[index].price} SAR",
                                    onTap: () {
                                      setState(() {
                                        widget.entity.services.removeAt(index);
                                      });
                                    });
                              },
                            ...
                            builder: (context, state) {
                              if (state is OrderSummarySuccessState) {
                                return CustomTotalRow(
                                  price: "${state.entity.totalPrice} SAR",
                                );
                              } else {
                                return CustomTotalRow(
