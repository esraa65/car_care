...
class _OrderSummaryState extends State<OrderSummary> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => OrderSummaryCubit(getIt())..orderSummary(),
          ...
          return Scaffold(
              ...
              body: switch (state) {
                ...
                OrderSummaryInitialState() =>
                  const Center(child: CircularProgressIndicator()),
                OrderSummaryFailuireState(errorMessage: final message) =>
                  Center(
                    child: DefaultText(
                      text: message,
                ...
                OrderSummarySuccessState(entity: final order) => Column(
                    children: [
                      Expanded(
                        child: Padding(
                          ...
                          child: Column(
                            children: [
                              ...
                              CustomWorkShopDetails(
                                subtitle:
                                    "${DateTime.parse(order.createdAt).toLocal().toString().split(' ')[0]} ${DateTime.parse(order.createdAt).toLocal().toString().split(' ')[1]}",
                                title: "تاريخ الحجز",
                              ),
                              height(16),
                              CustomWorkShopDetails(
                                subtitle: order.car ?? 'السياره',
                                title: "السياره",
                              ),
                              height(16),
                              CustomWorkShopDetails(
                                subtitle: order.services.first.type,
                                title: "فئة الخدمة",
                              ),
                              height(16),
                              const Divider(),
                              height(16),
                              SizedBox(
                                height: order.services.length * 40 +
                                    (order.services.length - 1),
                                width: double.infinity,
                                child: ListView.separated(
                                  itemBuilder: (ctx, index) {
                                    return CustomRowWorkshopDetailsWithIcon(
                                        title: order.services[index].type,
                                        price:
                                  ...
                                  separatorBuilder: (ctx, index) => height(16),
                              ...
                              const Divider(),
                              height(16),
                              CustomTotalRow(price: "${order.totalPrice} SAR"),
                            ],
                          ),
                        ),
                      ),
